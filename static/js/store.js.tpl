...
DOMContentLoaded.addEventOrExecute(() => {
    ...
    {# Update discount badges for a given product container #}
    function ivanaUpdateDiscountBadges($container) {
        var $this_compare_price = $container.find(".js-compare-price-display");
        var $this_price = $container.find(".js-price-display");
        var $installment_container = $container.find(".js-product-payments-container");
        var $installment_text = $container.find(".js-max-installments-container");
        var $this_add_to_cart = $container.find(".js-prod-submit-form");

        var compare_price_value = $this_compare_price.html();
        var price_value = $this_price.html();

        const percentageDifference = window.moneyDifferenceCalculator.percentageDifferenceFromString(compare_price_value, price_value);
        const numericPercentageDifference = parseFloat(String(percentageDifference).replace(",", "."));
        const roundedPercentage = Number.isFinite(numericPercentageDifference) ? Math.round(numericPercentageDifference) : 0;
        const hasPercentageDiscount = roundedPercentage > 0;

        if(hasPercentageDiscount){
            $container.find(".js-offer-percentage").text(percentageDifference);
            $container.find(".js-offer-label").css("display" , "inline-flex");
            $container.find(".product-discount-overlay").addClass("ivana-active");
        }

        if ($this_compare_price.css("display") == "none" || !hasPercentageDiscount) {
            $container.find(".js-offer-label").hide();
            $container.find(".product-discount-overlay").removeClass("ivana-active");
        }
        if ($this_add_to_cart.hasClass("nostock")) {
            $container.find(".js-stock-label-private").show();
            $container.find(".js-offer-label").hide();
            $container.find(".product-discount-overlay").removeClass("ivana-active");
        }
        else {
            $container.find(".js-stock-label-private").hide();
        }
    ...
    {# Run discount badge check on initial page load, after Tiendanube native JS #}
    setTimeout(function() {
        var $productContainer = jQueryNuvem("#single-product");
        if ($productContainer.length) {
            ivanaUpdateDiscountBadges($productContainer);
        }
    }, 0);
    ...
    jQueryNuvem(document).on("change", ".js-variation-option", function(e) {
        ...
        ivanaUpdateDiscountBadges($this_product_container);
    });
